...
class TestNerClient(unittest.TestCase):
    '''
    { ents: [{ "city name": "Berlin"  }] --> input
    html: "<span>..." --> print output}
    '''
    ...
    def test_get_ents_returns_dictionary_given_nonempty_string_causes_nonempty_spacy_doc_ent(self):
        model = NerModelTestDouble('eng')
        model.returns_doc_ents([])
        ner = NamedEntityClient(model)
    ...
    def test_get_ents_given_spacy_PERSON_is_returned_serialized_to_Person(self):
        model = NerModelTestDouble('eng')
        doc_ents = [{'text': 'Junu Moon', 'label_': 'PERSON'}]
        model.returns_doc_ents(doc_ents)
        ner = NamedEntityClient(model)
        result = ner.get_ents("...")
        expected_result = {'ents': [{'ent': 'Junu Moon', 'label': 'Person'}], 'html': ""}
    ...
    def test_get_ents_given_spacy_GPE_is_returned_serialized_to_Location(self):
        model = NerModelTestDouble('eng')
    ...
    def test_get_ents_given_multiple_ents_serializes_all(self):
        ...
        ner = NamedEntityClient(model)
        result = ner.get_ents("...")
        expected_result = {'ents': [{'ent': 'Junu Moon', 'label': 'Person'},
                                    {'ent': 'Australia', 'label':'Location'}], 'html': ""}
        self.assertListEqual(result['ents'], expected_result['ents'])